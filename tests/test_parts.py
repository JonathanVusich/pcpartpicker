...
class PartTest(unittest.TestCase):
    ...
    def test_cpu_init(self):
        ...
        cpu = CPU(*args)
        self.assertEqual(cpu.brand, args[0])
        self.assertEqual(cpu.model, args[1])
        self.assertEqual(cpu.clock_speed, args[2])
        self.assertEqual(cpu.cores, args[3])
        self.assertEqual(cpu.tdp, args[4])
        self.assertEqual(cpu.price, args[5])

    def test_cpu_bad_init(self):
        args = ["Intel", 6700, ClockSpeed.from_ghz("4.4"), 4, 95, Money("230.00", USD)]
        with self.assertRaises(ValueError):
            _ = CPU(*args)
    ...
    def test_cpu_cooler_init(self):
        ...
        self.assertEqual(cpu_cooler.brand, args[0])
        self.assertEqual(cpu_cooler.model, args[1])
        self.assertEqual(cpu_cooler.fan_rpm, args[2])
        self.assertEqual(cpu_cooler.decibels, args[3])
        self.assertEqual(cpu_cooler.price, args[4])
    ...
    def test_motherboard_init(self):
        ...
        self.assertEqual(mobo.brand, args[0])
        self.assertEqual(mobo.model, args[1])
        self.assertEqual(mobo.socket, args[2])
        self.assertEqual(mobo.form_factor, args[3])
        self.assertEqual(mobo.ram_slots, args[4])
        self.assertEqual(mobo.max_ram, args[5])
        self.assertEqual(mobo.price, args[6])
    ...
    def test_memory_init(self):
        ...
        self.assertEqual(memory.module_type, args[2])
        self.assertEqual(memory.speed, args[3])
        self.assertEqual(memory.form_factor, args[4])
    ...
    def test_storage_drive_init(self):
        args = ["Seagate", "7200 RPM", "WD Black", "2.5 in", "HDD", None, Bytes.from_tb(4), Bytes.from_mb(256),
                Money("0.08", USD),
                Money("123.00", USD)]
        ...
        self.assertEqual(hdd.form_factor, args[3])
        self.assertEqual(hdd.type, args[4])
        self.assertEqual(hdd.platter_rpm, args[5])
        self.assertEqual(hdd.capacity, args[6])
        self.assertEqual(hdd.cache_amount, args[7])
        self.assertEqual(hdd.price_per_gb, args[8])
        self.assertEqual(hdd.price, args[9])

    def test_storage_drive_bad_init(self):
        args = ["Seagate", "WD", "Black", "2.5 in", 7200, None, Bytes.from_tb(4), Bytes.from_mb(256),
                Money("0.08", USD),
                Money("123.00", USD)]
        with self.assertRaises(ValueError):
            _ = StorageDrive(*args)
    ...
    def test_gpu_init(self):
        args = ["EVGA", "SCC", "RTX 2080", "T104", Bytes.from_gb(8), ClockSpeed.from_ghz(2), Money("300.00", USD)]
        gpu = GPU(*args)
        self.assertEqual(gpu.brand, args[0])
        self.assertEqual(gpu.model, args[1])
        self.assertEqual(gpu.model_line, args[2])
        self.assertEqual(gpu.chipset, args[3])
        self.assertEqual(gpu.memory_amount, args[4])
        self.assertEqual(gpu.core_clock, args[5])
        self.assertEqual(gpu.price, args[6])
    ...
    def test_psu_init(self):
        args = ["EVGA", "Supernova", "G1", "ATX", "80+ Gold", 800, "Fully modular", Money("120", USD)]
        psu = PSU(*args)
        self.assertEqual(psu.brand, args[0])
        self.assertEqual(psu.model, args[1])
        self.assertEqual(psu.model_line, args[2])
        self.assertEqual(psu.form_factor, args[3])
        self.assertEqual(psu.efficiency_rating, args[4])
        self.assertEqual(psu.watt_rating, args[5])
        self.assertEqual(psu.modular, args[6])
        self.assertEqual(psu.price, args[7])

    def test_psu_bad_init(self):
        args = ["EVGA", "Supernova", "G1", "ATX", 80, 800, "Fully modular", Money("120", USD)]
        with self.assertRaises(ValueError):
            _ = PSU(*args)

    def test_case_init(self):
        args = ["Lian Li", "1f2d", "ATX", 4, 4, None, Money("120", USD)]
        case = Case(*args)
        self.assertEqual(case.brand, args[0])
        self.assertEqual(case.model, args[1])
        self.assertEqual(case.form_factor, args[2])
        self.assertEqual(case.external_bays, args[3])
        self.assertEqual(case.internal_bays, args[4])
        self.assertEqual(case.psu_wattage, args[5])

    def test_case_bad_init(self):
        args = ["Lian Li", "1f2d", "ATX", 4, 4, "435", Money("120", USD)]
        with self.assertRaises(ValueError):
            _ = Case(*args)

    def test_fan_init(self):
        args = ["Cooler Master", "Hyper 212 Evo", "black", 120, RPM(800, 2500, 1800), CFM(4, 21, 16),
                Decibels(21, 45, 33),
                Money("23.34", USD)]
        fan = Fan(*args)
        self.assertEqual(fan.brand, args[0])
        self.assertEqual(fan.model, args[1])
        self.assertEqual(fan.color, args[2])
        self.assertEqual(fan.size, args[3])
        self.assertEqual(fan.rpm, args[4])
        self.assertEqual(fan.airflow, args[5])
        self.assertEqual(fan.decibels, args[6])
        self.assertEqual(fan.price, args[7])

    def test_fan_bad_init(self):
        args = ["Cooler Master", "Hyper 212 Evo", True, 120, RPM(800, 2500, 1800), CFM(4, 21, 16), Decibels(21, 45, 33),
                Money("23.34", USD)]
        with self.assertRaises(ValueError):
            _ = Fan(*args)

    def test_fan_controller_init(self):
        args = ["Cooler Master", "FR34", "5.25\'", 5, 12, Money("60.00", USD)]
        fan_controller = FanController(*args)
        self.assertEqual(fan_controller.brand, args[0])
        self.assertEqual(fan_controller.model, args[1])
        self.assertEqual(fan_controller.form_factor, args[2])
        self.assertEqual(fan_controller.channels, args[3])
        self.assertEqual(fan_controller.channel_wattage, args[4])
        self.assertEqual(fan_controller.price, args[5])
    ...
    def test_thermal_paste_init(self):
        args = ["Cooler Master", "Liquid Metal", 23, Money("23.45", USD)]
        thermal_paste = ThermalPaste(*args)
        self.assertEqual(thermal_paste.brand, args[0])
        self.assertEqual(thermal_paste.model, args[1])
        self.assertEqual(thermal_paste.amount, args[2])
        self.assertEqual(thermal_paste.price, args[3])
    ...
    def test_optical_drive_init(self):
        args = ["LG", "Bluray", 12, 24, 48, "12", "24", "48", Money("60", USD)]
        drive = OpticalDrive(*args)
        self.assertEqual(drive.brand, args[0])
        self.assertEqual(drive.model, args[1])
        self.assertEqual(drive.bluray_read_speed, args[2])
        self.assertEqual(drive.dvd_read_speed, args[3])
        self.assertEqual(drive.cd_read_speed, args[4])
        self.assertEqual(drive.bluray_write_speed, args[5])
        self.assertEqual(drive.dvd_write_speed, args[6])
        self.assertEqual(drive.cd_write_speed, args[7])
        self.assertEqual(drive.price, args[8])
    ...
    def test_soundcard_init(self):
        args = ["SoundBlaster", "Max", "ATX", 5.1, 2048, 231, 96000.0, Money("60")]
        soundcard = SoundCard(*args)
        self.assertEqual(soundcard.brand, args[0])
        self.assertEqual(soundcard.model, args[1])
        self.assertEqual(soundcard.chipset, args[2])
        self.assertEqual(soundcard.channels, args[3])
        self.assertEqual(soundcard.bitrate, args[4])
        self.assertEqual(soundcard.snr, args[5])
        self.assertEqual(soundcard.sample_rate, args[6])
        self.assertEqual(soundcard.price, args[7])

    def test_soundcard_bad_init(self):
        args = ["SoundBlaster", "Max", "ATX", 5.1, 2048, 231, "96000", Money("60")]
        with self.assertRaises(ValueError):
            _ = SoundCard(*args)

    def test_ethernet_card_init(self):
        args = ["LG", "Hispeed", "PCI-e x8", NetworkSpeed.from_gbits(1000), 2, Money("60", USD)]
        ethernet_card = EthernetCard(*args)
        self.assertEqual(ethernet_card.brand, args[0])
        self.assertEqual(ethernet_card.model, args[1])
        self.assertEqual(ethernet_card.interface, args[2])
        self.assertEqual(ethernet_card.port_speed, args[3])
        self.assertEqual(ethernet_card.port_number, args[4])
        self.assertEqual(ethernet_card.price, args[5])
    ...
    def test_wireless_card_init(self):
        args = ["Realtek", "Wifi", "PCI-e x8", "b/g/n/ac", Money("60", USD)]
        wireless_card = WirelessCard(*args)
        self.assertEqual(wireless_card.brand, args[0])
        self.assertEqual(wireless_card.model, args[1])
        self.assertEqual(wireless_card.interface, args[2])
        self.assertEqual(wireless_card.supported_protocols, args[3])
        self.assertEqual(wireless_card.price, args[4])

    def test_wireless_card_bad_init(self):
        args = ["Realtek", "Wifi", "PCI-e x8", 802.11, Money("60", USD)]
        with self.assertRaises(ValueError):
            _ = WirelessCard(*args)
    ...
    def test_monitor_init(self):
        ...
        self.assertEqual(monitor.resolution, args[2])
    ...
    def test_headphones_init(self):
        args = ["Audio-Technica", "ATX-M50x", "Overear", False, False, FrequencyResponse(48, 128000, 24000),
                Money("150.00", USD)]
        headphones = Headphones(*args)
        self.assertEqual(headphones.brand, args[0])
        self.assertEqual(headphones.model, args[1])
        self.assertEqual(headphones.type, args[2])
        self.assertEqual(headphones.has_microphone, args[3])
        self.assertEqual(headphones.is_wireless, args[4])
        self.assertEqual(headphones.frequency_response, args[5])
        self.assertEqual(headphones.price, args[6])

    def test_headphones_bad_init(self):
        args = ["Audio-Technica", "ATX-M50x", "Overear", False, 1, FrequencyResponse(48, 128000, 24000),
                Money("150.00", USD)]
        with self.assertRaises(ValueError):
            _ = Headphones(*args)
    ...
    def test_keyboard_init(self):
        ...
        self.assertEqual(keyboard.brand, args[0])
        self.assertEqual(keyboard.model, args[1])
        self.assertEqual(keyboard.style, args[2])
        self.assertEqual(keyboard.color, args[3])
        self.assertEqual(keyboard.switch_type, args[4])
        self.assertEqual(keyboard.backlight_type, args[5])
        self.assertEqual(keyboard.price, args[6])
    ...
    def test_mouse_init(self):
        ...
        mouse = Mouse(*args)
        self.assertEqual(mouse.brand, args[0])
        self.assertEqual(mouse.model, args[1])
        self.assertEqual(mouse.type, args[2])
        self.assertEqual(mouse.connection, args[3])
        self.assertEqual(mouse.color, args[4])
        self.assertEqual(mouse.price, args[5])
    ...
    def test_speakers_init(self):
        args = ["Logitech", "Speakers", 5.1, 89.0, FrequencyResponse(96, 48000, 21000), Money("80", USD)]
        speakers = Speakers(*args)
        self.assertEqual(speakers.brand, args[0])
        self.assertEqual(speakers.model, args[1])
        self.assertEqual(speakers.channel_configuration, args[2])
        self.assertEqual(speakers.wattage, args[3])
        self.assertEqual(speakers.frequency_response, args[4])
        self.assertEqual(speakers.price, args[5])

    def test_speakers_bad_init(self):
        args = ["Logitech", "Speakers", 5.1, "89", FrequencyResponse(96, 48000, 21000), Money("80", USD)]
        with self.assertRaises(ValueError):
            _ = Speakers(*args)

    def test_ups_init(self):
        args = ["PowerMaster", "UPS", 1200, 24, Money("200", USD)]
        ups = UPS(*args)
        self.assertEqual(ups.brand, args[0])
        self.assertEqual(ups.model, args[1])
        self.assertEqual(ups.watt_capacity, args[2])
        self.assertEqual(ups.va_capacity, args[3])
        self.assertEqual(ups.price, args[4])
